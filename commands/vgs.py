...
def get_group_volumes(lvm_logger: Logger):
    ...
    try:
        lvm_logger.get_logger().info(" ".join(volume_group_command_array))
        # Extract relevant information from the JSON output
        json_format_output = run_command(volume_group_command_array, lvm_logger)[
            'report'][0]['vg']
